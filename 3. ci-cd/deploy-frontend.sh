...
sed -e "s;%BUILD_NUMBER%;${BUILD_NUMBER};g" frontend-deployment-config.json > frontend-deployment-config-v_${BUILD_NUMBER}.json
aws ecs register-task-definition --family devops-app-task --cli-input-json file://frontend-deployment-config-v_${BUILD_NUMBER}.json --no-paginate
